...
class JarvisOverlay extends StatefulWidget {
  ...
  const JarvisOverlay({super.key, required this.screenHeight});
...
class _JarvisOverlayState extends State<JarvisOverlay>
  ...
  double get _screenHeight => widget.screenHeight;
  ...
  void _updateOuterRingAngle() {
    ...
    if (delta < 0) {
      _outerRingAngle += delta + 1.0;
    } else {
      _outerRingAngle += delta;
    }
  }
  ...
  void _updateDataRingAngle() {
    ...
    if (delta < 0) {
      _dataRingAngle += delta + 1.0;
    } else {
      _dataRingAngle += delta;
    }
  }
  ...
  void _updateInnerRingAngle() {
    ...
    if (delta < 0) {
      _innerRingAngle += delta + 1.0;
    } else {
      _innerRingAngle += delta;
    }
  }
